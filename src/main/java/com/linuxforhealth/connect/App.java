...
/**
 * The LinuxForHealth Connect application. LinuxForHealth Connect provides data
 * integration and processing routes for application integration. Apache Camel
 * is use to provide routing, mediation, and processing services for data
 * integrations.
 *
 * Application settings are stored in an application.properties file, located on
 * the classpath. Camel Routes are defined within the
 * {@link App#ROUTE_BUILDER_PACKAGE} which is scanned on application startup.
 */
public final class App {
	 ...
	 * @throws ReflectiveOperationException if an error occurs creating new
	 *                                      component instances
	 */
	private void bindBeans(Properties appProperties) throws ReflectiveOperationException {

		Set<String> beanPropertyKeys = appProperties.stringPropertyNames().stream()
				.filter(prop -> prop.startsWith(App.BEAN_PROPERTY_NAMESPACE)).collect(Collectors.toSet());
	...
	/**
	 * Loads properties from a well known external location in
	 * {@link App#EXTERNAL_PROPERTY_FILE_PATH}
	 * 
	...
	/**
	 * Loads property settings from environment variables. Properties are translated
	 * from environment variable names and values.
	 * 
	 * @return {@link Properties} object.
	 */
	private Properties loadEnvironmentProperties() {
		...
		System.getenv().entrySet().stream().filter(e -> e.getKey().toLowerCase().startsWith(ENV_NAMESPACE))
				.forEach(e -> {
	...
	/**
	 * Loads application properties from the classpath, external file, and
	 ...
	 * </ul>
	 *
	 * Environment variables are translated to a valid "property" format. Example:
	 * LFH_CONNECT_FOO=bar is translated to lfh.connect.foo=bar.
	 *
	 * Properties are registered with the camel context and returned from this
	 * method for bootstrap processing. This "double-evaluation" is required as the
	 * app has to configure some components prior to the camel context being
	 * available.
	 *
	...
	/**
	 * Configures encrypted property parsing using PropertiesComponent and Jasypt
	 * for encryption.
	 *
	 * @return {@link Properties} instance
	 * @throws IOException if an error occurs reading application.properties
	 ...
	 * </ol>
	 * 
	 * @throws ReflectiveOperationException if an error occurs creating Camel
	 *                                      component instances
	 */
